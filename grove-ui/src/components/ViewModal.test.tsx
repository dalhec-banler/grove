...
describe('ViewModal', () => {
  it('renders new view form when initial is null', () => {
    render(<ViewModal initial={null} allTags={['photo', 'doc']} onClose={vi.fn()} onSave={vi.fn()} groups={[]} />);
  ...
  it('renders edit form when initial view is provided', () => {
    ...
    render(<ViewModal initial={view} allTags={['photo', 'doc']} onClose={vi.fn()} onSave={vi.fn()} groups={[]} />);
    expect(screen.getByText('Edit view')).toBeTruthy();
    expect(screen.getByDisplayValue('Photos')).toBeTruthy();
  });

  it('renders save button', () => {
    render(<ViewModal initial={null} allTags={[]} onClose={vi.fn()} onSave={vi.fn()} groups={[]} />);
    expect(screen.getAllByText('Save').length).toBeGreaterThan(0);
  });

  it('calls onSave with name, tags, and color when form submitted', () => {
    const onSave = vi.fn();
    render(<ViewModal initial={null} allTags={['photo', 'doc']} onClose={vi.fn()} onSave={onSave} groups={[]} />);
    fireEvent.change(screen.getByPlaceholderText('photos'), { target: { value: 'My Photos' } });
    fireEvent.click(screen.getByText('+ photo'));
    fireEvent.click(screen.getByText('Save'));
    expect(onSave).toHaveBeenCalledWith('My Photos', ['photo'], '#3A6BC5', null);
  });

  it('calls onClose when cancel clicked', () => {
    const onClose = vi.fn();
    render(<ViewModal initial={null} allTags={[]} onClose={onClose} onSave={vi.fn()} groups={[]} />);
    fireEvent.click(screen.getByText('Cancel'));
    expect(onClose).toHaveBeenCalled();
  });

  it('disables save when name is empty', () => {
    render(<ViewModal initial={null} allTags={['photo']} onClose={vi.fn()} onSave={vi.fn()} groups={[]} />);
    fireEvent.click(screen.getByText('+ photo'));
